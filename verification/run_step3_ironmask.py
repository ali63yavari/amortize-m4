...
What it does:
  1. Generates IronMask .sage files that follow the operation schedule of
     the current gadgets.c implementation:
       ...
       - refresh_sni
       - linear_refresh
  2. Runs IronMask for selected properties and orders.
...
def main() -> int:
    ...
    ap.add_argument("--include-pini", action="store_true",
                    help="Also run PINI for isw_and. It may fail for this encoding.")
    ...
    for order in range(1, args.max_order + 1):
        ...
        # refresh_sni: SNI is the main target.
        jobs.append(Job(
            gadget="refresh_sni",
            prop="SNI",
            order=order,
            sage_file=refresh_path,
            transcript_file=transcripts_dir / f"ours_refresh_sni_sni_order{order}.txt",
            timeout_seconds=args.timeout,
        ))
